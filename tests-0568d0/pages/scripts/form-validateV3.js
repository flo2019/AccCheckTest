  ...
  function attachValidation(form, status) {
    form.addEventListener('submit', (event) => {
      ...
      if (nachname && norm(nachname.value) === '') {
        setError(nachname, 'Bitte geben Sie Ihren Nachnamen ein.');
        hasError = true;
      }
      ...
      if (telefon) {
        ...
        if (val !== '' && !telefonPattern.test(val)) {
          setError(telefon, 'Bitte geben Sie eine gültige Telefonnummer ein.');
          hasError = true;
        }
      }
      ...
      if (betreff && betreff.required && norm(betreff.value) === '') {
        setError(betreff, 'Bitte wählen Sie einen Betreff aus.');
        hasError = true;
      }
      ...
      if (einwilligung && !einwilligung.checked) {
        setError(einwilligung, 'Bitte stimmen Sie der Verarbeitung Ihrer Daten zu.');
        hasError = true;
      }

      // Zentrale Meldung + Fokus
      if (hasError) {
        if (status) status.textContent = 'Bitte korrigieren Sie die markierten Fehler im Formular.';
